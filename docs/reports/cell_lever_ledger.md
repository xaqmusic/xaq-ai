...
| **R4 — competence-graded arbitration** (`LoopCompetence` per loop → `LateralVoter` level 1, `trust_source expected`, `group_balance false` → `scoring_mode precision`; `the_cell_route_far_pillars__r4_competence.json`) | study-brain base, far-food + pillars, falloff 2.0, n = 20 | **`NULL` (first form, the competence mean)** — 1.70 eats vs base 1.50 (Δ +0.20, sd 1.28, t 0.7); 0.75 below the two-loop brain (t −1.9); 1.10 below the specialist (t −2.5); = the floor. **Its wrong-sign control does not regress** (1.65, Δ +0.05, t 0.2), so the trust ordering is not what decides eats. Probe: play's competence falls below the prior (its novelty rarely rises over a window), klino's rises when it drives — and the planner, never given the motor, is never observed: its competence stays at the prior 0.5 and it loses every tie to klino (share 0.13). Preference dominates when trusts sit near uniform (0.18–0.34). Competence = the fraction of 30-tick driving windows on which the loop's objective moved as it predicts; stagnation = failure; non-drivers relax to 0.5. | The unobserved-competence problem is a bandit problem: grade by the posterior's **upper bound** (optimism), so an untried loop is tried and a proven-poor one is not — the second form, below. |
| **R4 — second form, `estimator beta` + `optimism κ`** | same, n = 20 | **`PARTIAL`, not a capability.** The decision shares reshape exactly as designed — play 0.89 (efe) → 0.51 at κ = 1 → 0.39 at κ = 2, the planner 0.06 → 0.20 → 0.23 — and eats follow the shares weakly: κ = 1 1.60 (Δ +0.10 vs base, t 0.4), κ = 2 1.95 (Δ +0.45, sd 1.19, t 1.7, 11+/6−), still 0.50 below the two-loop brain (t −1.2) and 0.85 below the specialist (t −2.6). **The wrong-sign control at κ = 1 does not regress (1.80)**: what moves eats is how much of the motor is taken from play, not which loop receives it. Across every arbitration arm today (R3, R4, R4b) eats rise as play's share falls and no ordering principle adds a detectable effect at n = 20. | The machinery (a generic competence grader with optimism, a voter, the arbiter's precision mode) is the recipe's arbitration for the duck; its Cell verdict is that with a novelty loop in the race, presence — not weight or ranking — costs the eats. Re-use: a world where novelty and food coincide, or a loop set without a novelty policy. |
