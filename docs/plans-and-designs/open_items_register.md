...
| O3 | **Value race vs precision-weighted arbitration.** The shipped `efe` scoring is hunger × reach in shared units plus a need-gated epistemic term; whether that counts as the precision weighting doctrine §2.3 prescribes, or is the mis-scaled proxy §2.2 forbids, is decided by the reach-gate ablation (Cell round 2 A1) and a follow-on lever putting the loops under `LateralVoter` trust. The duck's rung-2 fork (§17.4) is the same question. | `IN_FLIGHT` (A1) then `OPEN` | recipe §3 | doctrine §2.2–2.3; `cell_efe_arbiter_plan.md`; `microduck_rung2_regime_design.md` §17.4 |
