...
| 10 | The two launcher shell scripts are the only ones in the repo — bash-specific (`set -euo pipefail`, `${BASH_SOURCE[0]}`, `${PYTHONPATH:+:$PYTHONPATH}`). No native Windows port exists; a `.ps1` would need to be hand-written, not translated line-for-line, since they also assume `.venv/bin/` (POSIX layout) rather than `.venv/Scripts/`. They are near-identical, so one `.ps1` pattern covers both. | `tools/run_inspector.sh` (34 lines), `tools/run_voice_studio.sh` (same shape) |
| 10b | `tools/xaq_voice` is ALSA-only (`<alsa/asoundlib.h>`, `snd_pcm_*`) with no audio abstraction. Windows would need a WASAPI or PortAudio backend behind the same `audio_thread` seam; the DSP and patch layers are portable C++ and would not move. Its Qt front end has no extra Windows problem beyond row 10. | `tools/xaq_voice/src/main.cpp` (`audio_thread`), `tools/xaq_voice/CMakeLists.txt` (`pkg_check_modules(ALSA REQUIRED alsa)`) |
