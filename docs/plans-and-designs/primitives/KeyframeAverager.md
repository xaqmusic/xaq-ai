...
Per `docs/patent_abstract.md` §0024–§0026, EPMs are nominally allowed to operate at any time horizon. Phase v5.2 builds the missing primitive: a downsampler that buffers high-rate inputs in a rolling window and exposes the per-tick mean, which a sub-rate EPM (`process_every_n_ticks`) can consume.
