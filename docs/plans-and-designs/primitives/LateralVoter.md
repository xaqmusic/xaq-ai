...
1. **Trust-weighted fusion of latents.** Each subscriber's contribution to the fused embedding is weighted by `1/(tle + ε)` (low TLE → high trust), with **modality-group balancing** so a high-cardinality group (e.g. multiple visual EPMs) doesn't drown out a low-cardinality group (e.g. one audio EPM).
...
With one `reality.video.retinal`, one `reality.video.saliency`, one `reality.audio.stft`, and one `reality.proprio.imu`, no single group's combined trust exceeds 30% across 10⁴ ticks of randomized TLE inputs (with `group_balance == true`).
