...
| `trust_source` | string | HotMutable | `default` | `default`/`tle`/`quant_error`/`expected` | Which error the precision is computed from ([charter](../epm_kalman_lessons_plan.md), Stage 2). `default` = each path's own source (legacy: `tle`; informativeness path: `quant_error`). `expected` = `RealityToken::expected_error`, the channel's running expected TLE (the EPM's `ema_tle`), its innovation spread. The winner node's own running residual was tried and refuted: a noisy channel tiles its noise finely, so every node's own error is small. |
| `trust_power` | double | HotMutable | 1.0 | any | Precision `= 1/(err+ε)^power`. 1 = legacy, bit-identical. **2 = inverse variance, the Kalman weighting**: on the bench's two-sensor pair (noise ratio 3) trust on the clean sensor 0.67 → 0.81 (optimum 0.90), fused error ÷1.36 to within 8 % of the optimum, trust flicker ÷4.3 with `expected`; under the informativeness gate it repairs half of the gate's noisy-channel inversion. −1 = the wrong-sign control (×3.2 worse). Alone it makes a dead channel *more* trusted (its expected error → 0); pair it with `activity_gain`, which strips dead channels while this one weighs live ones. |
