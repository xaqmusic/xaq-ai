...
ParamSchema PlaceNav::params_schema() const {
    return {
        ...
        {"vel_topic",     ParamMutability::ConstructionOnly, "Egomotion vel [right,fwd] -> path-integration odometry. WARNING: values[0] (lateral) is SIGN-SENSITIVE here -- it enters odo_x_/odo_y_ directly, and those quantise into the node id when pi_cell_size>0, so a flipped lateral axis silently builds a laterally mirrored map rather than failing. The two bodies do NOT agree on that sign: the Cell publishes vel_ego as [local_vel.x, -local_vel.z] (body_controller.gd), so its forward is -Z and +X really is right; the picrawler forward is +Z, so its +X is anatomically LEFT (LEG NAMING MIRROR, picrawler_body.gd). Only Cell configs wire this today. Before pointing it at the picrawler, fix the lateral sign -- do not just repoint the topic.", ParamValue{std::string("reality.proprio.vel_ego")}},
