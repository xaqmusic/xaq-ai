...
double get_double(ParamValue const& v, std::string const& key) {
    ...
    throw std::invalid_argument("Premotor param '" + key + "' must be numeric");
}
int64_t get_int(ParamValue const& v, std::string const& key) {
    if (auto p = std::get_if<int64_t>(&v)) return *p;
    throw std::invalid_argument("Premotor param '" + key + "' must be integer");
}
bool get_bool(ParamValue const& v, std::string const& key) {
    if (auto p = std::get_if<bool>(&v)) return *p;
    throw std::invalid_argument("Premotor param '" + key + "' must be bool");
}
std::string get_string(ParamValue const& v, std::string const& key) {
    if (auto p = std::get_if<std::string>(&v)) return *p;
    throw std::invalid_argument("Premotor param '" + key + "' must be string");
}

void validate_phase_commit_mode(std::string const& mode) {
    if (mode == "none" || mode == "switch_penalty" || mode == "bin_boundary") return;
    throw std::invalid_argument(
        "Premotor param 'phase_commit_mode' must be one of: none, switch_penalty, bin_boundary");
}

} // namespace

Premotor::Premotor()  = default;
Premotor::~Premotor() = default;

std::string_view Premotor::type_name() const { return "Premotor"; }

std::vector<TopicSpec> Premotor::input_topics() const {
...
std::vector<TopicSpec> Premotor::output_topics() const {
    std::vector<TopicSpec> v{
...
ParamSchema Premotor::params_schema() const {
    return {
...
float Premotor::chosen_window_entropy() const {
...
void Premotor::on_setup(Bus* bus, ParamMap const& params) {
    bus_ = bus;
...
void Premotor::on_param_change(std::string_view key, ParamValue const& value) {
    std::string k(key);
...
void Premotor::handle_consensus(std::string_view /*topic*/, MessagePtr payload) {
...
void Premotor::handle_drive(std::string_view /*topic*/, MessagePtr payload) {
    if (!input_allowed(payload->producer_id)) return;
...
void Premotor::handle_neuro(std::string_view /*topic*/, MessagePtr payload) {
    if (!input_allowed(payload->producer_id)) return;
...
void Premotor::handle_event(std::string_view topic, MessagePtr payload) {
    if (!input_allowed(payload->producer_id)) return;
...
void Premotor::handle_bc_reflex_left(std::string_view /*topic*/, MessagePtr payload) {
    if (!input_allowed(payload->producer_id)) return;
...
void Premotor::handle_bc_reflex_right(std::string_view /*topic*/, MessagePtr payload) {
    if (!input_allowed(payload->producer_id)) return;
    auto a = std::dynamic_pointer_cast<const ActionOut>(payload);
    if (!a) return;
...
void Premotor::handle_bc_reflex_uni(std::string_view /*topic*/, MessagePtr payload) {
    if (!input_allowed(payload->producer_id)) return;
    auto a = std::dynamic_pointer_cast<const ActionOut>(payload);
    if (!a) return;
...
void Premotor::handle_intent_override(std::string_view /*topic*/, MessagePtr payload) {
    if (!input_allowed(payload->producer_id)) return;
...
// the latest state.
void Premotor::handle_explore_directive(std::string_view /*topic*/, MessagePtr payload) {
    if (!input_allowed(payload->producer_id)) return;
...
void Premotor::handle_phase_context(std::string_view /*topic*/, MessagePtr payload) {
    if (!input_allowed(payload->producer_id)) return;
...
void Premotor::apply_bc_update(uint64_t tick_id) {
...
void Premotor::apply_reward(float intensity) {
    if (!weights_initialised_) return;
...
void Premotor::finalize_mc_episode() {
...
void Premotor::tick(uint64_t tick_id) {
    int N = int(intent_accels_.size());
...
nlohmann::json Premotor::snapshot_state() const {
...
void Premotor::restore_state(nlohmann::json const& s) {
    if (s.is_null() || s.empty()) return;
    int version = s.value("version", 0);
    if (version != 1) {
        throw std::runtime_error("Premotor::restore_state: unknown version " +
