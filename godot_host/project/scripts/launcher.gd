...
const _PICRAWLER_CONFIG_ALLOWLIST: Array = [
	...
	"the_picrawler_motor_epm_embed_corridor_v3base__ga__bodypose__m1auth.json",  # ── ★ M1 AUTHOR · THE MASK-AUTHORING SLOW LOOP ── author_mode=1: the planner PROPOSES region masks (residual-guided where the raw decode systematically hallucinates, plus random exploration; depths 5-34 only — k<5 is reflex territory), TRIALS each for 800 ticks under a TRUE dual-cone counterfactual, and KEEPS masks scored PER-JOINT (author_score=1, the ledger's altitude): target-joint final/raw < 0.95 AND whole-body < 1.0 (the no-damage guard — the demo-mask failure mode). Whole-body scoring (author_score=0) was measured near-null first: dilution over 12 joints hides 8-25% target wins. WATCH in motor_planner: the magenta rect hops per trial (the author searching); the readout's AUTHOR line carries phase/trial/last-ratio + the KEPT list — an honest empty list means no region-separable excess mass exists on this vocabulary. Zero authority; behavior IDENTICAL to V3 BASE.
