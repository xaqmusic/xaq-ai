...
const _PICRAWLER_CONFIG_ALLOWLIST: Array = [
	...
	"the_picrawler_motor_epm_embed_corridor_v3base__ga__bodypose__m1auth__planpull__native_measured__honestjoints.json",  # ── P-c · HONEST JOINTS (sim-honesty step (c), 2026-09-11) ── native_measured with `joints` carrying the SERVO FORWARD MODEL of the command instead of achieved hinge angles — the only joint signal an encoder-less robot can have. WORKING (signal, n=6×6000 corridor): plv_w 0.133→0.200 (t=+14.2), steps 74→119, step_cv_real 0.836→0.673, net_z 6.01→6.70 (t=+2.0). Costs straight 0.786→0.743, scrub and tilt +23%. ⚠ motor_tle RISES 0.229→0.263 — the body takes 61% more steps, so that is liveliness, not a worse signal; on that metric alone the arm reads REGRESSION, which is why it is not judged alone. ⚠ Pins gym_mode=corridor + difficulty 0.3 (its measurement context); the parent benchmark declares arena, so a default-launched native_measured is a DIFFERENT WORLD, not an A/B control. Lever declared in metadata.body_env. NOT PROMOTED — never yet watched.
	"the_picrawler_motor_epm_embed_corridor_v3base__ga__bodypose__m1auth__planpull__native_measured__tofboom.json",  # ── P-d · ToF BOOM (step (d) sensor fidelity, 2026-09-13) ── models the AS-BUILT belly sensor: on a boom 70 mm aft at +77 mm (top of the HAT), ray along body-down, lever arm and tilt resolved out with the FUSED attitude. ⚠ THE POINT IS WHAT IT CANNOT SEE: measured 31.6 mm against a 5.7 mm belly truth (+25.9 mm, r=0.80) and it MISSED 21/21 belly-grounding samples. It therefore looks like it cures the height ratchet (h_keff 0.32 vs 0.92, peak 0.071 vs 0.118) purely by not observing the grounding that drives it. Watch gc_raw (the boom, what the brain gets) against gc_belly (truth ray; -1 when off) while the belly scrapes. Tilt comp cannot close it — the boom reads a DIFFERENT PATCH OF GROUND. Forces CHASSIS_COLLIDE=1 so the belly can actually touch. A/B control: native_measured, arena, same body.
