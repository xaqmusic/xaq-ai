...
const _PICRAWLER_CONFIG_ALLOWLIST: Array = [
	...
	"the_picrawler_motor_epm_embed_corridor_v3base__ga__bodypose__m1auth.json",  # ── ★ M1 AUTHOR · RUNG (a): THE CLOSED PREDICTION LOOP ── author_mode=1 + author_apply=1: the planner PROPOSES region masks (residual-guided where the raw decode systematically hallucinates; depths 5-34 — k<5 is reflex territory), TRIALS each for 800 ticks, KEEPS per-joint (target final/base < 0.95 AND whole-body < 1.0 no-damage guard), and the KEPT set now OPERATES — applied to the roll continuously in chronological keep order. Candidates are judged MARGINALLY (vs the kept-only BASE cone) so they can't inherit the kept set's credit; the bands score the BASE operating roll (trial churn excluded). WATCH in motor_planner: bright magenta rect = the live trial (hops); faint steady rects = the EARNED set operating; AUTHOR readout line shows APPLYING + suppression count. Control arm = author_apply 0 (slider). Zero authority; behavior IDENTICAL to V3 BASE.
	"the_picrawler_motor_epm_embed_corridor_v3base__ga__bodypose__m1auth__planpull.json",  # ── ★★★ LEVER (b) · PLAN PULL @ 0.05 — PROMOTED (operator UI review 2026-08-14: better walking stability, slight rear-leg gain, and markedly better 30° corridor-wall climbing). The stack's new operating point. ── The planner's BASE roll decode at depth 8 publishes as a per-leg objective; a joint pulls ONLY where its verified authority band holds (weight 1) and the pull fuses per-joint with the keyframe objective at plan_gain=0.05 — the MEASURED operating point: the dose response is an inverted-U (n=20: net_z 5.40→6.41, falls 8→2, tilt_sd 0.164→0.098 vs control; 0.1 TIES control — a whisper cooperates, a shout fights the keyframe loop). Distress cuts all plan weights: reflexes own emergencies. The body acts to FULFIL its own earned prediction — and because the roll is the KEPT-MASK-shaped BASE, earned inhibition now has a behavioral path. WATCH: does the gait gain coherence (step rhythm, straightness) without falls/belly cost; does the FR leg change when a rest-region mask is hand-applied on the [O] bench? Consumer meter pl_pull/pl_w in the body log; publisher meter plan.pp. A/B lever = plan_gain (0 = byte-identical).
	"the_picrawler_motor_epm_embed_corridor_v3base__ga__bodypose__m1auth__planpull__j1s4.json",  # ── ★★★ JOB #1 WINNER · THE EVOLVER'S OWN OPERATING POINT (2026-08-22) ── the first gain vector this project did not choose by hand. Found by the GainEvolver from the operator's baked point (seed 4 of 4, 30 generations), then judged at n=20 arena / 12k / solid chassis against that hand point re-run as a fresh control. RESULT: the falls tail is GONE — 4/20 seeds fall at the hand point, 0/20 here — while net_disp is unchanged (10.55 vs 10.40) and the loaded rear touchdowns HOLD (rlt 3605 vs 3847, rpt 12298 vs 12154), which the rival candidate j1s1 gave away (rlt −43%). ★ THE LOUD RESULT IS CONSISTENCY, not the mean: across-seed spread collapses on every metric — amp_min sd 0.111→0.018 (F=37.8), tilt_sd 0.107→0.036 (F=9.1), straight 0.103→0.045 (F=5.3), net_disp 2.10→1.20 (F=3.1), all significant. The hand point works well on most seeds and fails on a few; this one works on all of them. ⚠ The MEANS improve only marginally (paired t 1.8–2.0, p≈0.06–0.09, ns) and step_bal is slightly worse (0.14→0.09) — so this is a ROBUSTNESS win, not a speed win. WATCH: does it look as steady as the numbers say, and does the rear landing still load? The [U] panel shows the vector; σ is 0 so it will not drift while you watch.
	"the_picrawler_motor_epm_embed_corridor_v3base__ga__bodypose__m1auth__planpull__gainevo_live.json",  # ── ★★★ WATCH IT SEARCH · LIVE EVOLVER FROM THE HAND POINT (2026-08-22) ── the companion to __j1s4: that one shows you the DESTINATION, this one shows you the JOURNEY. Body starts exactly at the hand-found baked point and the GainEvolver runs live from boot (σ 0.08), so the settling is watchable instead of inferred from logs. Warmup is 16000 rather than 10000 on the operator's own observation that gait and loading need ~15k ticks to stabilise — below that the first generations judge a still-learning body and the search gets credited for the BODY's learning curve. Generation = 8000 ticks, so the first accept/revert lands near tick 24000 and roughly every 2.7 min after. WATCH on [U]: incumbent markers drifting off their dashed seed ticks, the accept/revert string, ge_ji falling; and rlt/rpt in the log must not collapse. Press [J] for a solid chassis (no config can set it) and set the seed in the launcher spinbox. ⚠ It will probably NOT land on j1s4's vector: the four job-1 seeds settled in different basins and their average was worse than any of them, so a fresh seed finding its own point is expected, not a reproduction failure.
	"the_picrawler_motor_epm_embed_corridor_v3base__ga__bodypose__m1auth__planpull__gainevo.json",  # ── PART IV · GAIN EVOLVER (2026-08-17, IN_FLIGHT) ── the promoted stack + the lifetime (1+1)-ES over the 8-gain vector (rear trio, amp_target, height_homeo, postural, coupling, plan), seeded at the operator's baked point. SHIPS AT mutation_sigma=0 = SILENT OBSERVER (byte-identical; the intrinsic criterion ge_ji is scored and mirrored but nothing publishes). A/B lever = the [U] panel's mutation sigma slider (0.08 = the search lives; σ=0 pauses for hand-tuning via [M], ADOPT hands your point back to the evolver). WATCH: ge_ji (error-form J, lower=better) must FALL as it searches; accept/revert history on [U]; ga_app lockstep with ge_pub×8 and ga_rej=0 (the two-sided consumer check); rlt/rpt must NOT collapse (the loaded rear landings are the thing job #1 must not give back). Criterion = falls + upright-variance + distress duty + unloaded-contact + (1−flow quality); guards = falls no-regression AND per-leg loaded minima, separate from J.
	"the_picrawler_motor_epm_embed_corridor_v3base__ga__bodypose__m1auth__planpull__pc5.json",  # ── OPTION B v2.1 · THE SURPRISE VOCABULARY, FED (all shadow) ── The promoted stack + the predictive-coding pair (body_pose_pc subtracts pc_predictor's latent-AR expectation, residual RMS-normalized, cap 800). CAMPAIGN STATE: structural gates PASS (self-mass 0.27 vs pose 0.69-0.72 — the stride is finally carved) but baked=0 at 24k — the co-learning predictor drifts the residual space forever; planner gates NULL so far. NEXT KNOB (zero code): pc_predictor freeze_after_ticks — converge then freeze → stationary residual → bakes → re-judge. WATCH in motor_planner_pc: do rear-knee (j10/j11) or h2 tracks earn bands the control planner lacks? Body-log gates: pc_n, pc_b (must leave 0), dp_err/dp_pn. Behavior IDENTICAL to the promoted stack.
