...
const _PICRAWLER_CONFIG_ALLOWLIST: Array = [
	...
	"the_picrawler_motor_epm_embed_corridor_imufused__stroke12__sm2.json",  # ── ★ PHASE-REFERENCE REPAIR — THE FIRST LOUD RESULT ── phase_vel_smooth=2. The Kuramoto coupling drives leg phases toward gait_phase offsets using L.phase = atan2(joint DELTA, pos−mean), and that delta was a RAW per-tick difference, i.e. a high-pass filter: near the zero crossings of a ~50-tick cycle its noise exceeds the position arm and the phase vector rattles instead of rotating. MEASURED: L.phase reversed direction on 31 % of ticks, so the coordination layer had been coupling to jitter for the entire campaign. A 2-tick average on the velocity arm alone takes wrong-way ticks 31 % → 18 %. ★ disp/s 0.1682 → 0.2032, +21.7 %, t=+5.60, 6/6 seeds — then REPLICATED on unseen seeds 7–12 at +17.0 %, t=+2.42, 5/6. Eleven of twelve seeds positive. Pulses get BIGGER (amp 0.243 → 0.262) rather than more regular (pulse_cv 0.583 → 0.633), so this buys stride POWER, not rhythm. ⚠ NON-MONOTONE: smooth 1 = +7.2 %, smooth 3 = +0.4 % — above ~2 the added lag cancels the gain, so this is a narrow optimum and not a "more is better" knob. phase_joint=hip1 is NOT the fix (wrong-way 37 %, disp/s −28 %). FOR THE EYE: does it look more purposeful, or merely faster?  # ── STROKE 1.20 ── the SYMPTOM treatment: less saturation → less of the commanded turn differential is rectified away by the clamp. Best BEHAVIOURAL arm: straight 0.474→0.488, net_z 4.58→4.70, asymmetry halved (1.44→0.69), 0 falls. ★ THIS ARM HAS A STANDING UI REQUEST IN THE LEDGER, unanswered since 2026-07-27: "PARTIAL awaiting operator UI observation — if the UI reads it as PURPOSEFUL rather than merely BUSIER, sweep 1.1-1.3 at n>=6." That is still the question.
