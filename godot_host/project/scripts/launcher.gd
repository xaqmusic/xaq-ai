...
const _PICRAWLER_CONFIG_ALLOWLIST: Array = [
	...
	"the_picrawler_motor_epm_embed_corridor_uprightgate.json",  # ── CURRENT ── the hardware-honest stack + homeo_upright_gate=0.5 on reality.proprio.upright. Normal walking BYTE-IDENTICAL (net_z 4.75, straight 0.74, tilt_sd 0.068) because upright~1.0 while walking means the gate never engages — but an inverted episode no longer poisons the walk: height_bias holds (-0.09 vs +1.50 railed) and amp_gain does not move (0.100 vs 2.53). Catastrophic forgetting here was an INTEGRATOR-WINDUP bug, not a learned-weights problem.
	# "the_picrawler_motor_epm_embed_corridor_imufused.json",  # same WITHOUT the upright gate — uncomment to A/B the forgetting failure directly (flip it in the pyramids and watch the walk never come back). FULLY HARDWARE-HONEST: commanded-angle IK ⊕ gyro-fused IMU attitude (no exact sim attitude anywhere). IMU sampled/filtered at the 240 Hz PHYSICS rate, not the 50 Hz brain tick — real IMUs run 1-8 kHz and the physics oversampling is free sensor bandwidth. Attitude error 3.17° mean. BEST ARM: net_z 3.76 (oracle) → 4.75, flat_v 0.04 → 0.05, straight 0.70 → 0.74, tilt_sd 0.067 → 0.068 (wobble gap CLOSED), hump 5.21 → 6.09, 0 falls. Beats even the exact-attitude arm (4.36).
