...
const _PICRAWLER_CONFIG_ALLOWLIST: Array = [
	...
	"the_picrawler_motor_epm_embed_corridor_v3base__ga__bodypose__m1auth__planpull__native_cad.json",  # ── ★★ NATIVE BENCHMARK · CAD BODY (stage E3a, 2026-08-26) ── the cad body's OWN operating point, settled by the evolver itself (free 3-gain search, σ 0.2, C2 criterion) from the j1s4 start and selected by the registered rule: amp 0.186 / coupling 0.878 / postural 1.262 (J3 1.765, 2 falls/600k while SEARCHING — the cleanest run of the arm). Evolver frozen (σ 0) as silent observer scoring C2 (travel = stride_v). metadata.body pins the geometry. PERMANENT DEMO PAIR with native_measured: same criterion, same seeds, different bodies → different discovered points (separation lives in amp+postural, not coupling). WATCH: [U] shows the vector; the HUD stride line shows fk/fuse/true live.
	"the_picrawler_motor_epm_embed_corridor_v3base__ga__bodypose__m1auth__planpull__native_measured.json",  # ── ★★ NATIVE BENCHMARK · MEASURED BODY (stage E3a, 2026-08-26) ── the as-built body's OWN operating point (same protocol/selection as native_cad): amp 0.307 / coupling 0.920 / postural 0.852 (J3 1.993; the measured arm searched through heavy falls — median 405/600k — so judge the POINT by this frozen config, not the search). metadata.body loads measured.json (hip_z +15%, l1 −14%, l2 −10%, mass +14%, real CoM + chassis boxes). The other half of the two-body demo pair.
