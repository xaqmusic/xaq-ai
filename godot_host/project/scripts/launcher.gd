...
const _PICRAWLER_CONFIG_ALLOWLIST: Array = [
	...
	"the_picrawler_motor_epm_embed_corridor_v3base__ga__bodypose__m1auth__planpull__pc.json",  # ── OPTION B · THE SURPRISE VOCABULARY (all shadow) ── The promoted stack + the predictive-coding pair, closed over the motor path for the first time: body_pose_pc subtracts pc_predictor's phase-expected pose BEFORE its GNG (the doctrine §0 descending path), so the vocabulary tiles DEVIATION-FROM-RHYTHM — touchdown/load corrections (incl. the operator's named rear-knee planting failure, j10/j11) should become its densest regions. motor_planner_pc (pure shadow) beside the control planner = the twin protocol. WATCH in the roll (motor_planner_pc widget): do the pc bands reach where the control's never did — below k=8, past k=34, onto h2 and the REAR KNEES? Body-log gates: pc_n self-limits, pc self-mass < 0.55, dp_res < 1 and falling (the predictor actually absorbing the stride). Behavior IDENTICAL to the promoted stack.
