...
| Working on the **microduck** | [`microduck port plan`](docs/plans-and-designs/microduck_port_plan.md) — the MuJoCo host (`mj_host`), the duck's sensorimotor surface, and **two tracks**: framework work at the joints, and a community contribution at the intent boundary that augments Pollen's RL stack rather than competing with it. **Design-stage; nothing built.** Branch `microduck` |
