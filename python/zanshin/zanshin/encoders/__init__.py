"""Zanshin encoder factory + registry bootstrap.
...
the private AMI-Awen cochlear encoder) plug in through the ``zanshin.encoders``
...
* with the AMI-Awen wheel installed, ``make_encoder("cochlear")`` resolves to
  the bio-mimetic Hopf encoder automatically — with zero code change in Zanshin.
"""
...
def _load_plugins() -> None:
    """Load external encoder providers via the ``zanshin.encoders`` group."""
    try:
        from importlib.metadata import entry_points
    except Exception:  # pragma: no cover
        return
    try:
        eps = entry_points(group="zanshin.encoders")
    except TypeError:  # Python < 3.10 API
        eps = entry_points().get("zanshin.encoders", [])
    for ep in eps:
        try:
            ep.load()(register)
        except Exception as exc:  # a broken plugin must not break Zanshin
            import warnings
            warnings.warn(f"zanshin.encoders plugin '{ep.name}' failed: {exc}")
